...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-043..INT-045 | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; ordinary messages are still deferred while an active handler waits; macro-generated async handler lowering still adapted through standard `Future` | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`; REQ-063 session matching has direct out-of-order response evidence; REQ-062 and macro-native ctx-future handler lowering pending | VAL-004, VAL-005 | implemented |
