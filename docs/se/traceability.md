...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-018A, INT-018B, INT-027, INT-043..INT-045 | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `crates/mpi/src/runtime.rs::block_on_ctx_task_with_dispatch`, `crates/mpi/src/runtime.rs::block_on_handler`, endpoint-backed task-local runtime receive, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; handler declarations use ordinary `fn` syntax; generated no-receiver handlers may use context-scoped `with_state` access for user state; generated event handlers for direct awaited assignment, await-and-discard, pre-await side effects followed by awaited assignment, deferred future binding followed by awaited assignment, awaited-let reply projection, two pending call futures, one stream-next await, and a recognized stream-next accumulator loop are lowered through native `CtxFuture` schedulers; unsupported awaited event-handler bodies now fail macro expansion instead of silently using the standard-future compatibility path | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`, `crates/mpi/tests/task_macro.rs`, `crates/mpi/tests/scope_compile_fail.rs`; REQ-064 scoped state access is covered by `req_064_generated_context_with_state_scopes_user_state_access` and `req_064_with_state_rejects_task_operation_inside_state_borrow`; REQ-062 generated-dispatch progress is covered by `req_062_generated_task_receives_call_request_while_handler_is_suspended`, `req_062_generated_pre_await_handler_dispatches_ordinary_message_while_suspended`, `req_062_generated_two_await_handler_dispatches_ordinary_message_while_suspended`, and `req_062_generated_stream_next_handler_dispatches_ordinary_message_while_suspended`; `req_062_req_064_unsupported_event_await_body_fails_instead_of_fallback` covers the no-silent-fallback guard; REQ-063 session matching has direct out-of-order response evidence; arbitrary awaited handler body lowering remains pending as a design/implementation decision | VAL-001, VAL-004, VAL-005 | implemented |
...
GAP-006: Handler declarations now use ordinary `fn` syntax. The `ctx-future` crate still owns the standard-`Future` compatibility adapter needed for non-awaiting handler bodies and external blocking compatibility paths. The runtime has an ordinary-message dispatch hook for native `CtxFuture` handlers, producer-side stream yield operations can send stream replies while returning control to that scheduler, and several generated event-handler shapes now use native `CtxFuture` schedulers. The unignored REQ-062 tests cover generated request progress for direct awaited assignment, pre-await side effects followed by awaited assignment, two pending call futures, and a stream-next accumulator loop. Awaited event-handler bodies that are not recognized by generated lowering now fail macro expansion instead of falling back to `block_on_handler`. Fully general arbitrary handler-body lowering into native `CtxFuture` continuations remains pending as a design/implementation decision.
