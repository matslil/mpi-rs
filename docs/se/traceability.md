...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-043..INT-045 | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `crates/mpi/src/runtime.rs::block_on_ctx_task_with_dispatch`, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; native `CtxFuture` handlers can dispatch ordinary messages while suspended; macro-generated async handler lowering still adapted through standard `Future` and still defers ordinary messages while the active handler waits | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`; REQ-062 has runtime-level ordinary dispatch evidence; REQ-063 session matching has direct out-of-order response evidence; macro-native ctx-future handler lowering pending | VAL-004, VAL-005 | implemented |
...
GAP-006: The `ctx-future` crate now has task-local runtime integration points in `mpi`, including an ordinary-message dispatch hook for native `CtxFuture` handlers, but macro-generated async handlers are still adapted through standard `Future`; full generated lowering into native `CtxFuture` continuations remains pending.
