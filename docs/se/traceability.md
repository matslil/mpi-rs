...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-043..INT-045, context receive API pending | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; ordinary messages are still deferred while an active handler waits; macro-generated async handler lowering still adapted through standard `Future` | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`; REQ-063 session matching has direct out-of-order response evidence; REQ-062 and macro-native ctx-future handler lowering pending | VAL-004, VAL-005 | implemented |
| REQ-070..REQ-072 | SN-011, SN-023 | CMP-011, ARCH-105, ARCH-107 | INT-040..INT-042, INT-009A, INT-042A | pending; protocol-declared receive source proposed in REQ-166 and REQ-168 | compile-fail/inspection pending | VAL-006 | approved |
...
GAP-001: Generated-code inspection evidence now covers the macro-produced message enum, handle methods, context type, placement implementation, spawn helper, dispatch arms, and protocol conversion plumbing. Compile-time receive declarations and native `CtxFuture` handler lowering remain separate implementation gaps.
