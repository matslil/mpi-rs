...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-043..INT-045 | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; generated dispatch now adapts async handlers into `CtxFuture` at the task-runtime boundary; ordinary messages are still deferred while an active handler waits | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`; REQ-063 session matching has direct out-of-order response evidence; REQ-062 concurrent ordinary-handler progress and native compiler lowering of arbitrary async handler bodies remain pending | VAL-004, VAL-005 | implemented |
...
GAP-006: The `ctx-future` crate now owns the standard-`Future` compatibility adapter, and `mpi` generated dispatch drives handlers through `CtxFuture`. Native lowering of arbitrary Rust `async fn` handler bodies into `CtxFuture` state machines remains pending because the public handler interface still relies on Rust async syntax.
