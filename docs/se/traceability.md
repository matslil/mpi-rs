...
| REQ-115 | SN-016, SN-043 | ARCH-077A | INT-075B | pending; current `yield_item()` and `yield_batch()` still return `SendError::StreamFlowLimited` when credit is unavailable | pending mapped test for no-credit yield suspension and resume after stream pull/cancel control | VAL-009 | approved |
...
GAP-009: REQ-115 has been approved to require `yield_item()` and `yield_batch()` to suspend under no-credit stream backpressure. Current implementation still returns `SendError::StreamFlowLimited`; the producer-side wait/resume implementation and mapped test remain pending.
