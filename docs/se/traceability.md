...
| REQ-060..REQ-064 | SN-015, SN-021, SN-042 | ARCH-040..ARCH-044, CMP-007, CMP-015 | INT-018A, INT-043..INT-045 | `crates/ctx-future`, `crates/mpi/src/runtime.rs::block_on_ctx_task`, `crates/mpi/src/runtime.rs::block_on_ctx_task_with_dispatch`, `crates/mpi/src/runtime.rs::block_on_handler`, `SuspendedCall` and `SuspendedStreamNext` ctx-future integration; handler declarations use ordinary `fn` syntax and generated dispatch enters the task-runtime `CtxFuture` boundary through `block_on_handler`; generated event handlers for direct awaited assignment, pre-await side effects followed by awaited assignment, awaited-let reply projection, and two pending call futures are lowered through `block_on_ctx_task_with_dispatch` so ordinary messages can dispatch while awaited operations are pending | `crates/ctx-future/tests/context_borrow.rs`, `crates/mpi/tests/runtime_baseline.rs`, `crates/mpi/tests/task_macro.rs`; REQ-062 generated-dispatch progress is covered by `req_062_generated_task_receives_call_request_while_handler_is_suspended`, `req_062_generated_pre_await_handler_dispatches_ordinary_message_while_suspended`, and `req_062_generated_two_await_handler_dispatches_ordinary_message_while_suspended`; REQ-063 session matching has direct out-of-order response evidence; native lowering of stream-next loops and arbitrary handler body shapes remains pending | VAL-001, VAL-004, VAL-005 | implemented |
...
GAP-006: Handler declarations now use ordinary `fn` syntax and generated dispatch enters the task-runtime `CtxFuture` boundary through `block_on_handler`. The `ctx-future` crate still owns the standard-`Future` compatibility adapter needed for handler bodies that are not lowered. The runtime has an ordinary-message dispatch hook for native `CtxFuture` handlers, producer-side stream yield operations can send stream replies while returning control to that scheduler, and several generated event-handler shapes now use that hook. The unignored REQ-062 tests cover generated request progress for direct awaited assignment, pre-await side effects followed by awaited assignment, and two pending call futures. Full generated lowering of stream-next loops and arbitrary handler bodies into native `CtxFuture` continuations remains pending.
