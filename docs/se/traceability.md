...
GAP-006: Handler declarations now use ordinary `fn` syntax and the `ctx-future` crate owns the standard-`Future` compatibility adapter used by generated dispatch. The runtime has an ordinary-message dispatch hook for native `CtxFuture` handlers, and producer-side stream yield operations can send stream replies while returning control to that scheduler, but generated dispatch still adapts normalized handlers through compiler async lowering before adapting to `CtxFuture`; full generated lowering into native `CtxFuture` continuations remains pending.
