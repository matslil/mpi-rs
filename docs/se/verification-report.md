...
| Task endpoint and handle lifecycle | test, inspection | `crates/mpi/src/task.rs`, `crates/mpi/tests/runtime_baseline.rs` | passed | `TaskHandle` wraps a shared `TaskEndpoint` that owns queue reference, endpoint ID, external session sequence, and accepting lifecycle. Tests cover shared endpoint direct enqueue, closed-endpoint send failure, and clone-shared external session allocation. |
